...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
    ...
    */
    ...
    public Iterable<V> directConnections(V vertex) {       
        int i=0;
        ArrayList <V> vert = new ArrayList<>();
        int x=vertices.indexOf(vertex);
        for (i=0; i<=vertices.size(); i++){
            if(edgeMatrix[x][i]!=null){
            vert.add(vertices.get(i));
            }   
        }
        return vert;
    }
    ...
    public Iterable<E> outgoingEdges(V vertex) {
        int i=0;
        ArrayList <E> edge = new ArrayList<>();
        int x=vertices.indexOf(vertex);
        for (i=0; i<=numVertices; i++){
            if(edgeMatrix[x][i]!=null){
            edge.add(getEdge(vertex, vertices.get(i)));
            }   
        }
        return edge;
    }
