...
public class GraphAlgorithms {
    ...
    public static <V, E> AdjacencyMatrixGraph<V, E> transitiveClosure(AdjacencyMatrixGraph<V, E> graph, E dummyEdge){
        ...
        AdjacencyMatrixGraph<V, E> gclone = new AdjacencyMatrixGraph<>();
        gclone = (AdjacencyMatrixGraph)graph.clone();
        
        int i=0;
        int k=0;
        int j=0;
        for (k=0; k<gclone.numVertices; k++){
            for (i=0; i<gclone.numVertices; i++){
                if(i != k && gclone.edgeMatrix[i][k]!= null){
                    for (j=0; j<gclone.numVertices; j++){
                        if (i != j && k != j && gclone.edgeMatrix[k][j] != null){
                           gclone.edgeMatrix[i][j]=dummyEdge; 
                        }
                    }
                }
            }
